...
def chang_page(driver,num):
    ...
    if cnum != str(num):
        val = driver.find_element_by_xpath('//ul[@class="ewb-com-items"]/li[1]/div/a').get_attribute(
            "href")[-30:]

        driver.execute_script("ShowAjaxNewPage(window.location.pathname,'categorypagingcontent',{})".format(num))

        locator = (By.XPATH, '//ul[@class="ewb-com-items"]/li[1]/div/a[not(contains(@href,"%s"))]' % val)
        WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator))
