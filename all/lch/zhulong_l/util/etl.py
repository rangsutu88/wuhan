...
def insert_tb(tbname, diqu, conp):
    ...
    sql2 = """
    insert into %s.gg
    select  distinct on (name,href ) name,href,credit_code,address,juridical_person,%s::text as ggtype,%s::text as jytype,
    '%s'::text diqu, info from %s.%s
    where (name,href) not in(select name,href from %s.gg)
