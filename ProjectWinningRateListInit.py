...
def get_content(driver):
    ...
    for i in range(0,len(tables)//2):
        ...
        corpName_link = tr.xpath('./td[2]/div/a/@onclick')[0]
        corpName = tr.xpath('./td[2]/div/a/text()')[0]
        corpName_link=MAIN_URL+re.findall(r"'.+'",corpName_link)[0].strip(r"'")

        tr=tables[i+len(tables)//2]
        xh = tr.xpath('./td[1]/div/text()')[0]
        corpCode = tr.xpath('./td[2]/div/text()')[0]
...
if __name__ == '__main__':
    ...
    url = 'http://www.whzbtb.cn/V2PRTS/ProjectWinningRateListInit.do'
    driver.get(url)
    change_page(driver)
