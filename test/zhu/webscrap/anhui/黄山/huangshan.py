...
def f1(driver,num):
    ...
    if cnum != str(num):
        ...
        # 第二个等待
        locator = (
        By.XPATH, '(//div[@class="content"])[2]/div/table/tbody/tr[1]/td[2]/a[not(contains(string(),"%s"))]' % val)
        WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator))
...
def f3(driver,url):
    ...
    locator = (By.XPATH, '/html/body/div[4]/div/div[2]/table/tbody/tr[4]/td')
...
def f5(driver,num):
    ...
    WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator))
    url = driver.current_url
    if ('004001007005' in url) or ('004001007006' in url):
        if num <= 51:
            main_url = url.rsplit('=', maxsplit=1)[0]
            cnum = re.findall(r'Paging=(\d+)', url)[0]
            url = main_url + '=' + str(num)
            ...
            num = num -51
            main_url='http://www.hscgw.gov.cn/hsweb/jyxx/004001/004001007/004001007006/?Paging'
            if num == 52:
                cnum='1'
            else:
                cnum=re.findall(r'Paging=(\d+)', url)[0]
            url=main_url+ '=' + str(num)
    elif ('004002003' in url) or ('004002004' in url):
        if num <= 289:
            main_url = url.rsplit('=', maxsplit=1)[0]
            cnum = re.findall(r'Paging=(\d+)', url)[0]
            url = main_url + '=' + str(num)
            ...
            num = num - 289
            main_url='http://www.hscgw.gov.cn/hsweb/jyxx/004002/004002004/?Paging'
            if num == 290:
                cnum='1'
            else:
                cnum=re.findall(r'Paging=(\d+)', url)[0]
...
def f6(driver):
    ...
    locator = (By.XPATH, '(//div[@class="content"])[2]/div/table/tbody/tr[1]/td[2]/a')
    WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator))
    if '004001007005' in url:
        page = driver.find_element_by_xpath('//td[@class="huifont"]').text

        total_1 = re.findall('/(\d+)', page)[0]
        total_1 = int(total_1)

        driver.get('http://www.hscgw.gov.cn/hsweb/jyxx/004001/004001007/004001007006/?Paging=1')
        total_2=f2(driver)
    ...
    if '004002003' in url:
        ...
        total_1 = re.findall('/(\d+)', page)[0]
        total_1 = int(total_1)
        driver.get('http://www.hscgw.gov.cn/hsweb/jyxx/004002/004002004/?Paging=1')
        total_2 = f2(driver)
        total = total_1 + total_2
        driver.quit()
        return total
...
data=[
    ...
    ["gcjs_xiaoxing_zhongbiaohx_gg","http://www.hscgw.gov.cn/hsweb/jyxx/004001/004001007/004001007005/?Paging=1",["name","ggstart_time","href","info"],f5,f6],
    #跟gcjs_xiaoxing_zhongbiaohx_gg合并
    ...
    ["zfcg_zhaobiao_gg","http://www.hscgw.gov.cn/hsweb/jyxx/004002/004002003/?Paging=1",["name","ggstart_time","href","info"],f5,f6],
    #跟zfcg_zhaobiao_gg合并
    # ["zfcg_zhaobiao_danyixing_gg","http://www.hscgw.gov.cn/hsweb/jyxx/004002/004002004/?Paging=1",["name","ggstart_time","href","info"],f1,f2],
