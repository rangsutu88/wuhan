...
def chang_address(driver,i,c_text):
    ...
    if cc_text != c_text:
        try:
            ...
            val = driver.find_element_by_xpath('//table[@class="ewb-table-info"]/tbody/tr[1]/td[2]/a').text
        ...
        try:
            ...
            locator = (By.XPATH, '//table[@class="ewb-table-info"]/tbody/tr[1]/td[2]/a[not(contains(string(),"{}"))] '.format(val))
    ...
    for i in range(1, 17):
        if i != 1:
            ...
            driver.find_element_by_xpath('(//div[@class="ewb-screen-list"])[1]/span[{}]'.format(i)).click()
                #
            try:
                locator = (By.XPATH, '//table[@class="ewb-table-info"]/tbody/tr[1]/td[2]/a[not(contains(string(),"%s"))]' % val)
                WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator))
                locator = (By.XPATH, '//span[@class="pg_maxpagenum"]')
                WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator))
                time.sleep(2)
