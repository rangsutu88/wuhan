...
def get_content(driver):
    ...
    for i in range(0,len(tables)//2):
        ...
        corpName_link = None if  tr.xpath('./td[2]/div/a/@onclick')==[] else tr.xpath('./td[2]/div/a/@onclick')[0]
        corpName = None if  tr.xpath('./td[2]/div/a/text()')==[] else tr.xpath('./td[2]/div/a/text()')[0]
        corpName_link=MAIN_URL+re.findall(r"'(.+)'",str(corpName_link))[0].split(r"','")[0]

        tr=tables[i+len(tables)//2]
        xh = tr.xpath('./td[1]/div/text()')[0]
        corpCode = None if tr.xpath('./td[2]/div/text()') == [] else tr.xpath('./td[2]/div/text()')[0]
        winningCount = tr.xpath('./td[3]/div/text()')[0]
        publicityEndDate = tr.xpath('./td[4]/div/text()')[0]

        try:
            content_two=callback_parse(corpName_link)
        except:
            content_two=None
            print('获取{}详情页失败'.format(corpName_link))

        content_dict['corpName_link']=corpName_link
        content_dict['corpName']=corpName
        content_dict['xh']=xh
        content_dict['corpCode']=corpCode
        content_dict['winningCount']=winningCount
        content_dict['publicityEndDate']=publicityEndDate
...
if __name__ == '__main__':
    ...
    url = 'http://www.whzbtb.cn/V2PRTS/CompanyWinningRateListInit.do'
    # driver.get(url)
