    ...
    for i in range(1, int(len(PAGE)) + 1):
        if sum(PAGE[:i - 1]) < num <= sum(PAGE[:i]):
            ...
            for content in contents:
                ...
                tmp = [name, address, ggstart_time, type, gg_type, jy_type, org_name, org_code, href]
                data_.append(tmp)
...
data=[
    ...
    ["zfcg_gg","http://www.ccgp-hunan.gov.cn/page/notice/more.jsp?noticeTypeID=prcmNotices",
     ["name", "address", "ggstart_time", "type", "gg_type", "jy_type", "org_name", "org_code",  "href",'info'],f1,f2],
