...
def f1(driver,num):
    ...
    if int(cnum) != num:
        ...
        val = driver.find_element_by_xpath('//div[@class="default_pgContainer"]/ul/li[1]/a').text
        url = main_url + '=' + str(num)

        driver.get(url)

        locator = (By.XPATH, '//div[@class="default_pgContainer"]/ul/li[1]/a[not(contains(string(),"%s"))]' % val)
        WebDriverWait(driver, 10).until(EC.presence_of_element_located(locator))
